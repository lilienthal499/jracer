...
jracer.controller.CarController = function (car) {
	...
	function steeringWheelTurnedRight() {
		delayedControllers.steeringWheel = new jracer.controller.DelayedController(400, function (progress) {
	...
	function steeringWheelNotTurned() {
		delayedControllers.steeringWheel = new jracer.controller.DelayedController(600, function (progress) {
			if (car.controls.steeringWheel > 0) {
				car.controls.steeringWheel -= progress;
				car.controls.steeringWheel = car.controls.steeringWheel > 0 ? car.controls.steeringWheel : 0;
			}
			if (car.controls.steeringWheel < 0) {
				car.controls.steeringWheel += progress;
				car.controls.steeringWheel = car.controls.steeringWheel < 0 ? car.controls.steeringWheel : 0;
	...
	function onKeyUpPressed() {
		delayedControllers.gasPedal = new jracer.controller.DelayedController(400, function (progress) {
			// This could be non-linear
	...
	function onKeyDownPressed() {
		delayedControllers.brake = new jracer.controller.DelayedController(200, function (progress) {
	...
	this.update = function () {
		...
		for (propertyName in delayedControllers) {
			if (delayedControllers.hasOwnProperty(propertyName) && delayedControllers[propertyName] !== undefined) {
...
jracer.controller.KeyboardController = function (keyConfig, carController) {
	...
	function setupKeys() {
	    for (index = keyConfig.length - 1; index >= 0; index = index - 1) {
			keys.push({ name: keyConfig[index].key, code: keyConfig[index].code, isPressed: false });
		}
	}
