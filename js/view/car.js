import { createDOMProxy } from './view.js';
...
function createCar(viewConfig, carModel) {
  ...
  const transform = createDOMProxy(DOMElement.style, 'transform');
...
export function MovingCar(viewConfig, carModel) {
  ...
  const left = createDOMProxy(car.getDOMElement().style, 'left');
  const bottom = createDOMProxy(car.getDOMElement().style, 'bottom');
